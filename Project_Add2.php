...
<div class="row div1 ">
    ...
    <div class="col-md-4"><leble>Engineer Name</leble></div>
     <div class="col-md-4">
        <select name="AddSite_engname" required>
            <option value="">-- Select Engineer</option>
            <?php 
            $engsql = mysqli_query($conn,"select distinct(engname) as engname from eng_master order by engname ASC") ; 
            while($engsql_result = mysqli_fetch_assoc($engsql)){
?>
    <option value="<?php echo $engsql_result['engname']; ?>">
...
<div class="row div1 ">
    ...
    <div class="col-md-4"><leble>Engineer Name</leble></div>
     <div class="col-md-4">
        <select name="Cloud_engineerName" required>
            <option value="">-- Select Engineer</option>
            <?php 
            $engsql = mysqli_query($conn,"select distinct(engname) as engname from eng_master order by engname ASC") ; 
            while($engsql_result = mysqli_fetch_assoc($engsql)){
...
let counter = i+1;
       ...
       html += " <div class=''><lable>IP Camera</lable> <select name='AddSite_DVRName[]' required><option value=''>Select Model</option><?php $model_sql = mysqli_query($conn,'select * from dvr_name where bankwise_show=1'); while($model_sql_result = mysqli_fetch_array($model_sql)){ echo "<option>$model_sql_result[1]</option>"; } ?> </select> </div>";
