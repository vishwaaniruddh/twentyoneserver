...
$previousDate = 'tem_'. date('dmY', strtotime('yesterday'));
$tableName = $previousDate . '_alert';

mysqli_query($conn,"DROP TABLE IF EXISTS $tableName"); 

$query = "CREATE TABLE $tableName ENGINE=InnoDB SELECT * FROM alerts WHERE DATE(receivedtime) = DATE_SUB(CURDATE(), INTERVAL 1 DAY)";
...
echo "CREATE table modified_alerts ENGINE=InnoDB
    SELECT
    SensorName,
    CASE
...
mysqli_query($conn,"
    CREATE table modified_alerts ENGINE=InnoDB
    SELECT
    SensorName,
    CASE
        WHEN alarm LIKE '%R' THEN 'Restoral'
...
$query = "SELECT primeid, SensorName, alarm_description, Customer, Bank, ATMID, SiteAddress, DVRIP, Panel_make, zon, City, State, id, panelid, createtime, receivedtime, comment, zone, alarm, closedBy, closedtime,reactive_status,sendip FROM modified_alerts";
