        ...
        if (in_array($language, self::$validLocaleLanguages)) {
            ...
            if ($locale != 'en_us') {
                ...
                foreach ($localeFunctions as $localeFunction) {
                    ...
                    $parts = explode('##', $localeFunction);
$localeFunction = $parts[0];
if (strpos($localeFunction, '=') !== false) {
    ...
    if (count($parts) === 2) {
        // Assign the first and second parts to $fName and $lfName
        $fName = trim($parts[0]);
        $lfName = trim($parts[1]);
        
        // Check if the function exists and the values are valid
            ...
            self::$localeFunctions[$fName] = $lfName;
                ...
                if (file_exists($configFile)) {
                    ...
                    foreach ($localeSettings as $localeSetting) {
                        // Split the line using '##' as the delimiter to strip out comments
                        $parts = explode('##', $localeSetting);
                        $localeSetting = trim($parts[0]); // Take the first part and trim it
                        
                        if (strpos($localeSetting, '=') !== false) {
                            // Split the setting into name and value
                            $settingParts = explode('=', $localeSetting);
                            if (count($settingParts) === 2) {
                                $settingName = strtoupper(trim($settingParts[0]));
                                $settingValue = trim($settingParts[1]);
    ...
    private static function checkMatrixOperands(&$operand1, &$operand2, $resize = 1)
    {
        ...
        if (!is_array($operand1)) {
            // Get matrix dimensions of operand2
            $dimensions = self::getMatrixDimensions($operand2);
            $matrixRows = $dimensions[0];
            $matrixColumns = $dimensions[1];
            
            // Fill operand1 with dimensions of operand2
            $operand1 = array_fill(0, $matrixRows, array_fill(0, $matrixColumns, $operand1));
            $resize = 0;
        } elseif (!is_array($operand2)) {
            // Get matrix dimensions of operand1
            $dimensions = self::getMatrixDimensions($operand1);
            $matrixRows = $dimensions[0];
            $matrixColumns = $dimensions[1];
            
            // Fill operand2 with dimensions of operand1
            $operand2 = array_fill(0, $matrixRows, array_fill(0, $matrixColumns, $operand2));
            $resize = 0;
        }
        
        // Get matrix dimensions of operand1
        $dimensions1 = self::getMatrixDimensions($operand1);
        $matrix1Rows = $dimensions1[0];
        $matrix1Columns = $dimensions1[1];
        
        // Get matrix dimensions of operand2
        $dimensions2 = self::getMatrixDimensions($operand2);
        $matrix2Rows = $dimensions2[0];
        $matrix2Columns = $dimensions2[1];
        
        // Check if matrices are compatible for resizing
    ...
    private function _parseFormula($formula, Cell $pCell = null)
    {
        ...
        while (true) {
            ...
            if ($opCharacter == '-' && !$expectingOperator) {                //    Is it a negation instead of a minus?
                ...
                $d = $stack->pop();
              // Check if each key exists in the array, use null if not
...
++$index;
                ...
                if (preg_match('/^' . self::CALCULATION_REGEXP_FUNCTION . '$/i', $val, $matches)) {
                    ...
                    $testPrevOp = $stack->last(1);
                    if ($testPrevOp !== null && $testPrevOp['value'] === ':') {
                        // Retrieve the start row/column reference from the last element in the output array
                        $startRowColRef = $output[count($output) - 1]['value'];
                    
                        // Extract sheet title and reference
                        $extractResult1 = Worksheet::extractSheetTitle($startRowColRef, true);
                        $rangeWS1 = $extractResult1[0];
                        $startRowColRef = $extractResult1[1];
                        ...
                        $extractResult2 = Worksheet::extractSheetTitle($val, true);
                        $rangeWS2 = $extractResult2[0];
                        $val = $extractResult2[1];
    ...
    private function processTokenStack($tokens, $cellID = null, Cell $pCell = null)
    {
        ...
        foreach ($tokens as $tokenData) {
            ...
            if ($this->branchPruningEnabled && isset($tokenData['onlyIf'])) {
                ...
                // Check if the key exists in the array and set to null if not
                if (isset($branchStore[$onlyIfStoreKey])) {
                    $storeValue = $branchStore[$onlyIfStoreKey];
                } else {
                    $storeValue = null;
                }
            
                // Handle storeValue as a boolean
                if ($storeValue === null) {
                    $storeValueAsBool = true;
                } else {
                    $storeValueAsBool = (bool) Functions::flattenSingleValue($storeValue);
                }
            
                // Handle array case for storeValue
                if (is_array($storeValue)) {
                    $wrappedItem = end($storeValue);
                    if (is_array($wrappedItem)) {
            ...
            if ($this->branchPruningEnabled && isset($tokenData['onlyIfNot'])) {
                ...
                // Check if the key exists in the array and set to null if not
                if (isset($branchStore[$onlyIfNotStoreKey])) {
                    $storeValue = $branchStore[$onlyIfNotStoreKey];
                } else {
                    $storeValue = null;
                }
...
$result = Worksheet::extractSheetTitle($operand1Data['reference'], true);

// Check if the result is an array with at least two elements
if (is_array($result) && count($result) >= 2) {
    // Assign the values manually
    $sheet1 = $result[0];
    $operand1Data['reference'] = $result[1];
} else {
    // Handle the case where the result is not as expected
    // e.g., assign default values or handle the error
    $sheet1 = null;
    $operand1Data['reference'] = $operand1Data['reference']; // Or some default value
}
                        ...
                        $result = Worksheet::extractSheetTitle($operand2Data['reference'], true);

                        if (is_array($result) && count($result) >= 2) {
                            $sheet2 = $result[0];
                            $operand2Data['reference'] = $result[1];
                        } else {
                            $sheet2 = null;
                            $operand2Data['reference'] = $operand2Data['reference'];
                        }
        ...
        if ($pSheet !== null) {
            ...
            if (strpos($pRange, '!') !== false) {
                ...
                if (is_array($result) && count($result) >= 2) {
                    ...
                    $pSheetName = null;
                    $pRange = null; // or set to a default value if appropriate
                }
    ...
    public function extractNamedRange(&$pRange = 'A1', Worksheet $pSheet = null, $resetLog = true)
    {
        ...
        if ($pSheet !== null) {
            ...
            $aReferences = Coordinate::extractAllCellReferencesInRange($pRange);
            if (!isset($aReferences[1])) {
                $coordinates = Coordinate::coordinateFromString($aReferences[0]);
                if (is_array($coordinates) && count($coordinates) >= 2) {
                    $currentCol = $coordinates[0];
                    $currentRow = $coordinates[1];
                } else {
                    $currentCol = null;
                    $currentRow = null; // Handle error or default value
                }
                ...
                foreach ($aReferences as $reference) {
                    $coordinates = Coordinate::coordinateFromString($reference);
                    if (is_array($coordinates) && count($coordinates) >= 2) {
                        $currentCol = $coordinates[0];
                        $currentRow = $coordinates[1];
                    } else {
