...
mysqli_query($cn,"LOCK TABLES Sites WRITE");


$chkAvailable= mysqli_query($cn,"select ATMID from Sites where ATMID='".$ATMID."' or NewPanelID='".$NewPanelID."' or PanelIP='".$PanelsIP."' or DVRIP='".$DVRIP."' and live Not IN ('N','Y') ");
...
$result3=mysqli_query($cont,$sql);


$last2=mysqli_insert_id($conn);
$last3=mysqli_insert_id($cont);
...
$AddSite_RouterIp=$_POST['AddSite_RouterIp'];
	...
	$sqlBroad="insert into broadbanddetails(site_id,NetworkType ,ProviderName ,ProviderEmail  ,ProviderMobile  ,InternetPlans ,BroadbandAmount ,BroadbandAddress ,RouterIp ,MonthPlans,StartInternetDate )
	values('".$last2."','".$NetworkType."','".$ProviderName."','".$ProviderEmail."','".$ProviderMobile."','".$InternetPlans."','".$BroadbandAmount."','".$BroadbandAddress."','".$AddSite_RouterIp."','".$MonthPlans."','".$StartInternetDate."')";
    $resultBroad=mysqli_query($conn,$sqlBroad);
//echo $sqlBroad;
//////////////////////////////////////////////////
...
//window.open("ProjectAdd_Demo.php", "_self");
</script>
<?php } }

 mysqli_query($cn,"UNLOCK TABLES");
...
$chkAvailable= mysqli_query($cn,"select ATMID from dvrsite where ATMID='".$ATMID."' ");
$n=mysqli_num_rows($chkAvailable);
...
$chkAvailable= mysqli_query($cn,"select ATMID from dvronline where ATMID='".$ATMID."' ");
$n=mysqli_num_rows($chkAvailable);
...
$sql="insert into dvronline(ATMID,Address,Location,State,IPAddress,`Rourt ID`,`Live Date`,UserName,Password,Status,dvrname,customer)values('".$ATMID."','".$SiteAddress."','".$ATMShortName."','".$fetch['state']."','".$DVRIP."','".$Router_Model_num."','".$curentdt."','".$CTS_UserName."','".$CTS_Password."','".$Status."','".$dvrname."','".$Customer."')";
