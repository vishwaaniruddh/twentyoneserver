...
$sql1="select sensorname as Description,camera from securico where (Zone='".$row["zone"]."')";
	...
	*/
	 ...
	 if(strpos($row["Panel_make"], 'SMART-IN') !== FALSE)
    {
...
//echo $sql1;
	...
	else if(strpos($row["Panel_make"], 'rass_sbi') !== FALSE)
    {
...
//echo $sql1;
    	...
    	else if(strpos($row["Panel_make"], 'rass_cloud') !== FALSE)
    {
...
//echo $sql1;
	...
	else if(strpos($row["Panel_make"], 'SMART') !== FALSE)
    {
    
$sql1="select SensorName as Description,Camera from smarti where (Zone='".$row["zone"]."' and SCODE='".$row['alarm']."')";

    }
	else if(strpos($row["Panel_make"], 'sec_sbi') !== FALSE)
    {
    
$sql1="select sensorname as Description,camera from sec_sbi where (Zone='".$row["zone"]."')";
...
$sql1="select sensorname as Description,camera from securico where (Zone='".$row["zone"]."')";
	...
	else if(strpos($row["Panel_make"], 'securico_gx4816') !== FALSE)
    {
