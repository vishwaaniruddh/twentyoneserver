...
$check_live = mysqli_query($conn,"select * from dvronline where SN='".$site_id."'");
...
if($check_live_result['Status']!=$live){
    $live_sql = "insert into live_info_details(site_id,table,status, created_at,created_by) 
                values('".$site_id."','dvronline','".$live."','".$datetime."','".$userid."')";
...
$qry="update dvronline set `Rourt ID` = '".$router_id."', UserName='".$UserName."',Password='".$Password."',customer='".$Customer."',Bank='".$Bank."',ATMID='".$ATMID."',ATMID2='".$ATMID2."',Address='".$SiteAddress."',State='".$State."',IPAddress='".$DVRIP."',dvrname='".$DVRName."',Status='".$LiveStatus."' where id='".$updateid."' ";
...
$details_sql = "insert into dvronline_details(dvrid,tracker,bmName,engineerName,snapshots,status,created_at,created_by,statusDate) values('".$updateid."','".$Cloud_trackerno."','".$Cloud_bmname."','".$Cloud_engineerName."','".$Cloud_livesnapshots."',1,'".$datetime."','".$userid."','".$statusDate."')";
